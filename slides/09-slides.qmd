...
- Tutorial and lab this week, as usual (due Monday April 6)

- Assignment 3, data exploration due Sunday April 5

- If you got your data ready in assignment 2, should be simple. 

- You will complete your data cleaning, create 1 descriptive figure, and complete a basic outline. 
...
- A [standard error]{.blue} is simply an estimate of the [standard deviation]{.blue} of a theoretical [sampling distribution]{.blue}
...
## Variance of Regression Coefficients: Intiution

- Last time,  discussed sampling distributions and how we can approximate them with a normal distribution (CLT). 

- For a regression coefficient, centered around $\beta$ and we need to estimate a $\sigma^2$ (variance).

- To do this, we use the residuals. 

- You do not need to understand all the matrix algebra. 
...
## Variance of Regression Coefficients: Robust SE

- So far, we assumed that variance is the same for every observation

- This let us estimate uncertainty using a single number: $\sigma^2$

- But in many real datasets, some observations are much noisier than others

- We need a way to estimate variance that allows variance to differ across observations


## Robust Standard Errors {.smaller}

Robust standard errors attempt to estimate $\sigma_i^2$ using the residuals from the model $\hat{\epsilon_i}$ and [additional adjustments](https://declaredesign.org/r/estimatr/articles/mathematical-notes.html) to yield [robust standard errors]{.blue} that are [consistent]{.blue}, even when there is heteroskedasiticity. 
...
- We will (finally) discuss how to use our coefficient estimates and standard errors to perform **statistical inference**

- We will cover **confidence intervals** and how they are related to **hypothesis tests**

- Lastly, discuss how these **asymptotic** approaches differ from other approaches to inference, such as simulation. 

*Asymptotic* refers to the use of assumptions about our estimator/ its sampling distribution as *n* goes to infinity (recall the Central Limit Theorem)
