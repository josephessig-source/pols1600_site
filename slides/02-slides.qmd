...
- Submit tutorials from last week by tonight.
...
Please upload tutorials 00-intro and 01-measurement1 to [Canvas](https://canvas.brown.edu/courses/1102109/assignments) by tonight. 
...
```{r}
new_packages <- the_packages[!(the_packages %in% installed.packages()[,"Package"])]
if(length(new_packages)) install.packages(new_packages)
invisible(lapply(the_packages, library, character.only = TRUE))
```
...
**NO! Of course not. For Pete's sake, It's only the second week**
