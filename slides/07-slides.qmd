...
- [Assignment 2  Data:](https://josephessig-source.github.io/pols1600_site/assignments/a2.html/) due Sunday March 15

- Lab 5 grades will be posted this week. 
...
Residualized regression is a way of understanding what it means to [control for variables]{.blue} in a regression.

Residualized regression provides a way of illustrating what we mean when we say the coefficients describe the unique variance in Y explained by some predictor $x$ (and only $x$)

This result is called the **Frisch-Waugh-Lovell (FWL) theorem**
...
![](../files/llustration-of-the-linear-regression-model-as-a-projection-method.png)
...
annotate(
  x = 0, y = -0.5,
  geom = "text", label = "beta[1]", size = 10, hjust = .5,parse=TRUE
) +
xlim(-5.5, 4.5) +
ylim(-4.2, 3.4) +
coord_equal()
...
annotate(
  x = -0.45, y = -.45,
  geom = "text", label = "beta[1]", size = 5, hjust = .5,parse=TRUE
) +
xlim(-5.5, 4.5) +
ylim(-4.2, 3.4) +
coord_equal()
...
- Might make interpretation easier (but the onus is on you to describe your models well)
  ...
  - Don't standardize binary predictors
...
- Led John Snow (the physician, not from Game of Thrones) to explore the origins of Cholera, eventually concluding that cholera was transmitted through living organisms in water.
...
Let's consider some strategies Snow could take to estimate this quantity:


## Before vs after comparisons:{.smaller}

:::{.nonincremental}
- Snow could have compared Lambeth in 1854 $(E[Y_i(1)|D_i = 1] = 19)$ to Lambeth in 1849 $(E[Y_i(0)|D_i = 1]=85)$, and claimed that moving the pumps upstream led to **66 fewer cholera deaths.** 
...
- In the canonical pre-post, treated and control DiD, $\beta_3$ from the following linear regression will give us the ATT:
...
snow_g %>%
  ...
  annotate(geom = "text", x = 0, y = 135,
           label = "beta[0]", parse=TRUE, hjust=0.5, vjust =-.5)+
...
- On Friday, we'll walk through 
  - the paper's design and argument
  - setting up and exploring the data
  - reproducing some descriptive figures

- Next Friday, we'll focus on replicating and understanding the main results
