...
Please upload tutorials 00-intro and 01-measurement1 to Canvas by Monday, February 2nd. 

## Hopes and Dreams, Fears and Worries

Class survey results: 

- Mostly no experience with stats, less with coding/R

- Worries:
  + Programming/Coding
  + Lack of experience
  + Less familiar with quant research 
...
## General Monday/Wednesday Workflow

-   Go to <https://josephessig-source.github.io/pols1600_site/>
...
-   Create .qmd file and save in course folder
...
One common ocurrence is that a function in R is expecting data of one type but (e.g. `numeric`) but actually gets data of different type (e.g. `character`)

The `class()` function is a useful `base R` for troubleshooting such errors. 

For example, different operatinos on data frames return different objects. 
...
#| echo: true
#| code-fold: false
is_it_the_weekend <- function(x){
  # Determine if I have to go to work today
  # If x is in the weekend
  if(x %in% c("Saturday","Sunday")){
    return("Yes, take it easy!")
  }else{
    return("Sorry, back to work.")
  }

}
is_it_the_weekend("Saturday")
is_it_the_weekend("Monday")
```
...
df_dta <- haven::read_dta("https://josephessig-source.github.io/pols1600_site/files/data/covid.rda")
```
...
#load() tells R to load a .rda file, which is one way to store data.
#In this case, the rda file is stored at a URL, not on your computer. 
