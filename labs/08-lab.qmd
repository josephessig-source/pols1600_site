...
1. **Set working directory and load the data** from last lab 

2. Do some additional **recoding** 

3. **Describe** variation in voting by state, year, policy, and age 

4. **Estimate** four regression models to understand **fixed effects** and **cluster robust standard errors** 

5.  **Replicate** two regression models from Grumbach and Hill (2021) **interacting** `sdr` with `age_group` 

6. **Recreate** a portion of Figure 3 showing the **marginal effect** of `sdr` by age group.

One of these tasks  from (3-6) will be randomly selected as the graded question for the lab.
...
Recall that when you are loading data, you will ordinarily have a file on your computer, and you will have to set the working directory using `setwd()` (or using session > set working directory > to source file location) to the correct folder and then load the file from your computer, using `load()`, `read.csv()`, or similar. To save time, I have provided you the data file to download directly. 
...
load(url("https://josephessig-source.github.io/pols1600_site/files/data/cps_clean.rda"))
```
...
**The code also creates the following additional variables, that will be useful for presenting and visualizing data:**
...
#Create a vector for presidential election years
...
Now you will create some figures to describe variation in the data across variou features.  

## Variation by state

This code creates a figure that shows how average turnout varies across state in the `cps` data
...
This code creates a figure that shows how turnout varies across time. 

It calculates the `year`ly averages separately by `election_type` and creates a plot facetted by `election_type` using `facet_wrap(~election_type)`
...
1. Figure out which states adopted `sdr`during the time period (this code is provided)
2. Create a summary dataframe of average turnout by age for a single state using `filter()`, `group_by()` and `summarise()` (this code is provided also)
3. Pipe this dataframe to `ggplot()` setting the appropriate aesthetics using `geom_point()` and `facet_grid()` (this is what you have to do yourself)

:::{.callout-tip}
Hint: Creating the Plot

Here is one way to create this plot. 

1. Starting with the `cps` data, using `filter()`, select a single state that adopted SDR at some point in the data (pick from the ones in `sdr_adopted`). For NH, you would write `filter(st=="NH")`

2. Pipe the filtered dataframe into `ggplot()`

3. using `aes()`, map `year` to x,  `turnout` to y, and `SDR` to color. 

4. Specify the type of plot using `geom_point()`

5. Calculate conditional means of turnout before and after SDR using `stat_smooth(method = "lm", formula = "y ~ 1")`

6. Separate by age group using `facet_grid(~ age_group)`
...
no_sdr_states <- unique(cps$st[cps$sdr == 0])
sdr_adopted <- intersect(no_sdr_states, sdr_states)

#This list will have some years SDR=0, and some years SDR=1. 
sdr_adopted

cps %>% 
  filter(st %in% sdr_adopted) %>% 
    filter(!is.na(age_group)) %>%
  group_by(st, year, SDR, age_group) %>% 
  summarise(
    turnout = mean(dv_voted,na.rm=T)
  ) %>%
    mutate(age_group = factor(as.character(age_group))) -> sdr_age_df
...
Please fit **two models** that **interact** the variable `sdr` with the variable `age_group` (`sdr*age_group`). Include fixed effects for `st`ate and `year` (using `fixed_effects = ~ st + year`) and cluster your standard errors by `st`ate (`clusters = st`). 
...
That's a lot of coefficients to sort through. 
...
To recreate Figure 3, we'll need to add the coefficients on the interactions of `sdr` with `age_group` to the coefficient on `sdr` to get the marginal effects for each age group.


## Calculate Marginal Effects of Interactions

In the code below is **custom function** called `me_fn()` to calculate the **marginal effects** of Same Day Registration, a specific age cohort^[This is not a robust function, but one designed to work specifically for these data and models]
...
![](https://josephessig-source.github.io/pols1600_site/labs/images/08_fig3.png)
