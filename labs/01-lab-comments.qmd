...
To view this document more easily and see the output, please render and view as an HTML document. 
...
2.  Create a new dataframe, called `covid_us`, by filtering out observations from the U.S. territories `- Use`\<-`,`%\>%`,`filter()`,`!`and`%in%\` 
...
-   Use `%>%`,`group_by()`,`mutate()` and `lag()`, \``/` and `*`

5.  Create a variable called `face_masks` from the `facial_coverings` variable.
...
-   $(x_{i} - \bar{x})^2$ squaring this ensures that we treat positive and negative deviations the same when calculating the overall variance
...
We can see that as the vaccination rate changes over time, the correlation between vaccination rate and new cases per capita is also changing over time. There seems to be no single stable relationship between percent vaccinated and the correlation between vaccination rate and new cases per capita. We can look at this further by also calculating the mean number of new cases per capita, in addition to the correlation. 
...
covid_us %>%
  ...
  summarise(
    mn_per_vax = mean(percent_vaccinated, na.rm=T),
    mn_newcases_pc = mean(new_cases_pc, na.rm=T),
    cor = cor(new_cases_pc, percent_vaccinated, use = "complete.obs")
  )

```

After doing this, it seems like  the rate of vaccination increases quickly at first, and there is at first a clear negative relationship between vaccination and new  the number of new cases in 2021. However, new cases continue to fluctuate up and down over time, and the relationship between percent vaccinated and new cases seems to disappear after the first year.

If we wanted to, we could continue to interpret this result further by looking at particular states one at a time, or by accounting for other factors that might confound the relationship between vaccination and new cases, which might include population density, behavior, policies, etc. 
...
The problem is, we don't get to see that counterfactual outcome. So how can we make causal claims about the effects of facemasks, or any other policy that interests us? Finding creative ways to answer these questions is the key to making credible causal claims, which we will hear more about later in the class. 
