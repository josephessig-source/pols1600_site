...
4.  Clean the data, creating the covid_us dataframe.
5.  Recode the data
  - create per capita measure of new cases
  - create measure of face mask policy
  - create year-month variable for summarizing
6.  Calculate the average number of new cases in a given month for states with a given policy on face masks
7.  Produce an initial figure
8..  Refine the figure
...
Loaded the packages we need to clean our data, and create the plot. 
...
There are lots of ways to do this. At a minimum
...
I'd probably use the code from the comments to [Lab 01](https://josephessig-source.github.io/pols1600_site/labs/01-lab-comments.html#replicate-the-data-cleaning-and-recoding-from-class){target="_blank"} as my guide.
...
## Clean the data, creating the covid_us dataframe. 
...
First, we created a territories object of states/territories to filter out from administrative_area_level_2.

Then, we took the covid dataframe, and used filter() exclude all of the data that has a value of administrative_area_level_2 in "territories" from the new dataframe covid_us. 

Finally, we checked that this filtering worked. 
...
Creating separate `year` and `month` variables aren't strictly necessary,
...
- Check out the examples from [Monday's slides](https://josephessig-source.github.io/pols1600_site/slides/02-slides#/dataviz-describing-distributions-and-associations)
...
If our goal was to make comparisons across face mask policies over a given time period, I might still prefer something closer to our original graph.
